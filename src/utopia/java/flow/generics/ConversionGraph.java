...
import utopia.java.flow.generics.ValueParser.ValueParseException;
import utopia.java.flow.structure.Graph;
import utopia.java.flow.structure.GraphEdge;
import utopia.java.flow.structure.GraphNode;
...
public class ConversionGraph
{
	...
	public Option<DataType> findOptimalTargetType(DataType from, SubTypeSet to)
	{
		...
		return ImmutableList.of(to).flatMap(type -> getConversionCost(from, type).map(
				cost -> new Pair<>(type, cost))).sortedBy(Pair::second).headOption().map(Pair::first);
	}
	...
	public Option<ConversionReliability> getConversionReliability(DataType from, DataType to)
	{
		...
		return findOptimalRouteBetween(from, to).map(ConversionRoute::getReliability);
	}
	...
	public Option<Integer> getConversionCost(DataType from, DataType to)
	{
		...
		return findOptimalRouteBetween(from, to).map(ConversionRoute::getCost);
	}
	...
	private static class ConversionRoute
	{
		...
		{
			return steps.map(s -> s.first().getReliability().getCost()).reduceOption(Integer::sum).getOrElse(0);
		}
	}
}
